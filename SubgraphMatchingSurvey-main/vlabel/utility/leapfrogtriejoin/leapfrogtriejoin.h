...
private:
    ...
    bool enumerate_non_core_results(uint32_t start_depth, uint32_t max_depth,double TimeL);
...
public:
    leapfrogtriejoin(flat_relation* input, flat_relation* output, uint32_t* vertex_ordering,
                     ...
                     uint64_t output_count_limit = std::numeric_limits<uint64_t>::max(),int TimeL=std::numeric_limits<int>::max()) :
                             ...
                             output_count_limit_(output_count_limit), materialize_(false), count_(0),timeL_(TimeL) {
        initialize();
        initialize_bn_fn();
    ...
    enumResult execute(size_t& call_count);
