...
protected:
    virtual void initialize(catalog* catalog, std::vector<flat_relation *> &relations, uint64_t output_count_limit,int TimeL) = 0;
...
public:
    ...
    enumResult execute(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit, size_t& call_count,int TimeL) {
        
        auto start = std::chrono::high_resolution_clock::now();
        initialize(&catalog, relations, output_count_limit,TimeL);
...
protected:
    void initialize(catalog *catalog, std::vector<flat_relation *> &relations, uint64_t output_count_limit,int TimeL) override;
