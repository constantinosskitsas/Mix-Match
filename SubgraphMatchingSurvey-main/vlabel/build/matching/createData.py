...
#for i in range(len(pathsize)):
    for j in range(len(timts)):
        ...
        with open(f'{FMPath}{algo[i]}_-1_patents_{timts[j]}_16_50{endf}', 'r') as file:
            for line in file:
                values = line.split()
                if(counter<10000):
