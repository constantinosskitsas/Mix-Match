...
void no_partition_hash_join_node::initialize(catalog *catalog, std::vector<flat_relation *> &relations,
                                             uint64_t output_count_limit,int TimeL) {
    assert(!is_leaf());
...
void leap_frog_trie_join_node::initialize(catalog *catalog, std::vector<flat_relation *> &relations, uint64_t output_count_limit,int TimeL) {
    if (!is_leaf()) {
    ...
    lftj_ = new leapfrogtriejoin(input_relation_, output_relation_, vertices_.data(), static_cast<uint32_t >(vertices_.size()),
                                     catalog->query_graph_->get2CoreSize(), catalog, output_count_limit,TimeL);
}

void leap_frog_trie_join_node::execute_operator(std::vector<flat_relation *> &relations, size_t& call_count) {
    enumOutput= lftj_->execute(call_count);
