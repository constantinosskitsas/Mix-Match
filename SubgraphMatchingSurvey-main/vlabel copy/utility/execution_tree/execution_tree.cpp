...
enumResult execution_tree::execute(catalog &catalog, uint64_t output_count_limit, size_t& call_count,double TimeL) {
    enumResult count ;
    for (auto node : tree_nodes_) {
        count = node->execute(catalog, relations_, output_count_limit, call_count,TimeL);
    }
    return count;
}